...
5. 若非 `private-local`：读 `docs/agent-library.md` 与 `docs/suggested-assets.md`

## Agent Library（复利）

- 共享库：`02_Resources Files/05_Agent-Library`（manifest 索引）
- 任务前可查询：`ide-toolbox/scripts/query-agent-assets.sh --task "<摘要>"`
- 第一次产出写回项目；第二次跨项目需要再晋升到库
- `private-local`：**禁止**读/写 `05_Agent-Library`
