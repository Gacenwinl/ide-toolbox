...
- **Agent CLI**：支持 `chat`/`continue`/`resume` 多轮对话；`milestone` 含写回验证；Plan/Subagent 见 `docs/agent-cli-modes-and-subagents.md`
- **自维护就绪**：关闭 Cursor IDE 后 `./agent chat .` 多轮讨论，`./agent milestone .` 写回
...
- Date: 2026-06-14
- Summary: README.html 操作教程写完整：开始前、日常三步、菜单、新建项目、防忘文档、复利、自我迭代、完全用 Tool、FAQ、速查卡片
- **Next agent（终端）**：
  1. `cd ide-toolbox`
  2. 多轮讨论：`./agent chat .` 或续聊 `./agent continue .`
  3. 只读接手：`./agent start .`
  4. 阶段结束：`./agent milestone . --summary "..."`
- **Next agent reads**: 本节 → `docs/agent-cli-modes-and-subagents.md` → `docs/agent-cli-self-maintenance.md`
...
- [x] `docs/ai-context.md` — Current State / Last Session
- [x] `README.md` / `README.html` — 操作教程与能力总览
- [x] `docs/changelog.md` — 本阶段变更
- [x] `automation-playbook.md` / `docs/scripts-reference.md` — 菜单与脚本一致
- [x] `docs/runbook.md` — 维护验证步骤（若流程有变需更新）
